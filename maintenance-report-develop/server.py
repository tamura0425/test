...
import logic.task as ta
import logic.projects as pr
...
class MainHandler(tornado.web.RequestHandler):
  ...
  def post(self):
    ...
    if result:
      # ログイン成功のため保守報告書画面に遷移
      pritem = pr.getProjects(options, pg_con, self)
      prtask = pr.getPrtask(options, pg_con, self)
      mtime = pr.getPrmtime(options, pg_con, self)
      wtime = pr.getPrtime(options, pg_con, self)
      self.render("home.html", pritem=pritem, prtask=prtask, mtime=mtime, wtime=wtime)
    else:
      # ログイン失敗
...
class TaskHandler(tornado.web.RequestHandler):
  def get(self):
    ...
    if result:
        if self.get_argument("id") == "0":
          taitem = ta.initTask()
          self.render("task.html", taitem=taitem)
        else:
          taitem = ta.getTask(options, pg_con, self)
          self.render("task.html", taitem=taitem)
  ...
  def post(self):
    #バリデーション
    taitem = ta.validatorTask(options, pg_con, self)
    
    if self.get_argument("taskid") == "0":
      result = ta.registTask(options, pg_con, self)
    else:
      result = ta.updateTask(options, pg_con, self)
...
#    result = au.sessionCheck(options, pg_con, self)
#    if result:
#      result = ta.taskedit(options, self, pg_con)
#      if result:
#        self.render("task.html")
#      else:
#        self.render("index.html", system_name=options.system_name, msg="System error.")
#    else:
#      self.render("index.html", system_name=options.system_name, msg="再ログインが必要です。")
